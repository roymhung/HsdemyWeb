...
<body class="sb-nav-fixed">
                            ...
                            <div class="row g-4 align-items-center">
                                <div class="col-lg-8">
                                    <div class="d-flex justify-content-between align-items-start flex-wrap gap-2 mb-2">
                                        ...
                                        <a href="/admin/course" class="btn btn-light btn-sm">Back to Course List</a>
                                    </div>
                                    <p class="mb-3 opacity-75">${course.shortDesc}</p>
                                    <div class="mb-3">
                                        <span class="meta-badge">Author: ${course.author}</span>
                                        <span class="meta-badge">Level: ${course.level}</span>
                                        <span class="meta-badge">Category: ${course.title}</span>
                                    </div>
                                    <h4 class="mb-0">
                    ...
                    <div class="row g-4">
                        <div class="col-xl-8">
                            <div class="card section-card mb-4">
                                <div class="card-header bg-white d-flex justify-content-between align-items-center">
                                    ...
                                    <button class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#addChapterModal">
                                ...
                                <div class="card-body">
                                    <c:forEach var="chapter" items="${chapters}">
                                        <div class="chapter-box">
                                            <div class="chapter-header">
                                                ...
                                                <span class="badge bg-dark">${chapter.lessons.size()}</span>
                                            </div>
                                            <div class="chapter-body">
                                                <div class="d-flex flex-wrap gap-2 mb-2">
                                                    <button type="button" class="btn btn-warning btn-sm open-topic-modal"
                                                    ...
                                                    <form action="/admin/course/${course.id}/chapter/${chapter.id}/delete" method="post"
                                                        class="d-inline"
                                                        onsubmit="return confirm('Bạn có chắc muốn xóa chapter này?');">
                                                        <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />
                                                        <button type="submit" class="btn btn-outline-danger btn-sm">Delete</button>
                                                    </form>
                                                    <span class="badge ${chapter.status == 'ACTIVE' ? 'bg-success' : 'bg-secondary'} ms-auto">
                                                        ${chapter.status}
                                                    </span>
                                                </div>
                                                <p class="text-muted mb-2">${empty chapter.description ? 'Chưa có mô tả chương.' : chapter.description}</p>

                                                <c:forEach var="topic" items="${chapter.lessons}">
                                                    <div class="topic-row d-flex justify-content-between align-items-center">
                                                        <button type="button"
                                                            ...
                                                            <span class="text-dark">
                                                                <strong>${chapter.position}.${topic.position}</strong> - ${topic.title}
                                                            </span>
                                                            <c:if test="${topic.preview}">
                                                                <span class="badge bg-success ms-1">Preview</span>
                                                            </c:if>
                                                        </button>
                                                        <span class="badge bg-light text-dark">${topic.duration} phút</span>
                                                    </div>
                                                </c:forEach>
                                                <c:if test="${empty chapter.lessons}">
                                                    <p class="mb-0 text-muted">Chưa có topic nào.</p>
                                                </c:if>
                                            </div>
                                        </div>
                                    </c:forEach>
                                    ...
                                    <c:if test="${empty chapters}">
                                        <p class="text-center text-muted mb-0">Chưa có chapter nào cho course này.</p>
                                    </c:if>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-xl-4">
                            <div class="card section-card">
                                <div class="card-header bg-white fw-semibold">Course Description</div>
                                <div class="card-body">
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form:form method="POST" action="/admin/course/${course.id}/chapter/create" modelAttribute="newChapter"
                        class="row g-3">
                        <div class="col-md-3">
                        ...
                        <div class="col-12">
                            <button type="submit" class="btn btn-warning">Add Course Section</button>
                        </div>
                    </form:form>
                </div>
            </div>
    ...
    <div class="modal fade" id="addTopicModal" tabindex="-1">
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header bg-warning">
                    <h5 class="modal-title">Add Section Topic</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <form id="addTopicForm" method="POST" enctype="multipart/form-data" class="row g-3">
                        ...
                        <div class="col-md-3">
                            <label class="form-label">Sr No</label>
                            <input id="topicPosition" name="position" type="number" min="1" class="form-control" required />
                            <div class="form-text">Hiển thị ngoài giao diện theo dạng chapter.topic (ví dụ 2.1)</div>
                        </div>
                        <div class="col-md-9">
                            <label class="form-label">Section Name</label>
                        ...
                        <div class="col-md-4">
                            ...
                            <input name="duration" type="number" min="0" class="form-control" value="0" />
                        </div>
                        <div class="col-md-4 d-flex align-items-end">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="preview" id="previewInline">
                                <label class="form-check-label" for="previewInline">Is Preview</label>
                            </div>
                        </div>
                        <div class="col-12">
                            <label class="form-label">Select Topic Video (mp4, mov)</label>
                            <input name="videoFile" type="file" accept=".mp4,.mov,video/mp4,video/quicktime"
                                class="form-control" required />
                        </div>
                        <div class="col-12">
                            <button type="submit" class="btn btn-warning">Add Section Topic</button>
                        </div>
                    </form>
                </div>
            </div>
